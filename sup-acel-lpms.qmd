...
## Linguagens de Programação para Sistemas Heterogêneos
...
Para entender como é paralelizado o programa, é crucial compreender quatro conceitos fundamentais do modelo de execução heterogênea:
...
### Compute Kernel e Hierarquia de Threads

Um kernel é uma função executada simultaneamente por milhares de threads na GPU. Diferente da CPU, projetada para baixa latência em tarefas sequenciais, a GPU é projetada para alto throughput em tarefas paralelas (modelo SIMT - Single Instruction, Multiple Threads).

Para gerenciar essa escala, as threads são organizadas hierarquicamente:

1.  **Thread**: A unidade fundamental. Cada thread executa o mesmo código do kernel, mas possui um ID único (índice) que usa para calcular qual pedaço dos dados deve processar.
2.  **Bloco (Block)**: Um agrupamento de threads que executam no mesmo multiprocessador. Threads no mesmo bloco podem compartilhar memória rápida (Shared Memory) e sincronizar entre si.
3.  **Grade (Grid)**: O conjunto total de blocos lançados para resolver o problema inteiro.

O programador deve definir as dimensões da Grade e dos Blocos para mapear o problema (ex: pixels de uma imagem) para o hardware.

- **Em C++ (CUDA)**: A função é marcada com `__global__` e chamada com `kernel<<<grid, block>>>(args)`.
- **Em Python (Numba)**: A função é decorada com `@cuda.jit` e chamada com `kernelgrid, block`.
- **Em Julia (CUDA.jl)**: Usa-se a macro `@cuda threads=... blocks=... kernel(args)`.
...
A chamada de um kernel pelo Host geralmente é assíncrona. Isso significa que a CPU envia o comando para a GPU e imediatamente continua a executar as próximas linhas de código, sem esperar a GPU terminar. Para ler os resultados corretos, é necessário usar mecanismos de **sincronização** (como `cudaDeviceSynchronize()` em C++ ou cópias de memória bloqueantes) para garantir que a GPU finalizou o trabalho antes de a CPU tentar acessar os dados.
...
### Tabela comparativa dos itens

| Critério | C++ (CUDA/SYCL) | Python (Numba/CuPy) | Julia (CUDA.jl) |
| :--- | :--- | :--- | :--- |
| **Curva de Aprendizado** | Alta (Complexa) | Baixa (Amigável) | Média |
| **Performance** | Máxima (Nativa) | Alta (via Backends) | Máxima (JIT Nativa) |
| **Portabilidade** | Média (SYCL ajuda) | Alta | Alta |
| **Uso Ideal** | Sistemas críticos/Jogos | IA / Prototipagem rápida | Ciência de Dados / HPC |

## Estudos de Caso em Ecossistemas Alternativos

Embora o objetivo desse curso seja instruir acerca da aceleração nas linguagens C++, Python e Julia. Existem outras formas de aceleração, e apesar delas não serem muito usuais, não tão práticas e com menor ganho de desempenho, ainda são aceleradores em linguagens modernas. Para que o curso não fique incompleto, resolvi ao menos fazer uma menção honrosa a elas.
...
Agora que já entendemos como funciona cada uma de nossas linguagens, aprendemos sobre os mecanismos por trás da aceleração e os diferentes meios utilizados por cada linguagem, vamos agora visualizar o resultado do código com a aceleração implementada.

### C++

Exemplo otimizado do C++, aqui foi usado o CUDA C++. Mais especificamente utilizando Memória Unificada(Unified Memory).
...
__global__ void multiply_matrices(int* A, int* B, int* C) {
...
void initialize_matrix(int* matrix, int size, int value_start) {
    for (int i = 0; i < size; ++i) {
        matrix[i] = value_start + i;
    }
}
...
int main() {
    ...
    cudaMallocManaged(&B, M * P * sizeof(int));
...
Para o Python, utilizaremos o **Numba**, que permite compilar funções Python para código de máquina (e GPU) em tempo de execução. Note como a estrutura do código muda para acomodar o paralelismo: o laço duplo `for` que percorria os pixels desaparece do kernel, sendo substituído pelo índice global da thread (`cuda.grid(2)`).
...
def mandelbrot_kernel(min_x, max_x, min_y, max_y, width, height, max_iter, image):
    # Mapeia a thread atual para as coordenadas (x, y) da imagem
    x, y = cuda.grid(2)
...
fractal_image = create_fractal_gpu(-2.0, 1.0, -1.0, 1.0, 1024, 768, 255)
Image.fromarray(fractal_image).save('mandelbrot_gpu.png')
```

### Julia

Em Julia, a alteração no código é necessária para utilizar a biblioteca `CUDA.jl`. Diferente de C++, não há um comando de compilação separado no terminal; a compilação ocorre automaticamente na primeira execução (JIT). Abaixo, adaptamos o exemplo de Jacobi para rodar na GPU. Note que transformamos o laço `for` em um cálculo de índice baseado na thread, similar ao CUDA C++.

```julia
using CUDA

# Kernel CUDA em Julia
function jacobi_kernel_gpu!(new_array, old_array)
    # Calcula o índice global da thread
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    
    # Verifica limites (ignorando as bordas conforme lógica original)
    if i > 1 && i < length(old_array)
        new_array[i] = (old_array[i-1] + old_array[i+1]) / 2.0
    end
    return
end
...
function run_jacobi_gpu(size, iterations)
    # Inicializa arrays na CPU
    A = zeros(Float64, size)
    A[1] = 100.0
    A[size] = 100.0
    
    # Move dados para a GPU (CuArray)
    A_d = CuArray(A)
    next_A_d = copy(A_d)
    ...
    for _ in 1:iterations
        # Lança o kernel com a macro @cuda
        @cuda threads=threads blocks=blocks jacobi_kernel_gpu!(next_A_d, A_d)
        
        # Troca os ponteiros (apenas referências, sem cópia de dados)
        A_d, next_A_d = next_A_d, A_d
    end
    
    # Traz o resultado de volta para a CPU
    return Array(A_d)
end

final_state = run_jacobi_gpu(4096, 1000)
println("Resultado no centro (GPU): ", final_state[div(4096, 2)])
```
